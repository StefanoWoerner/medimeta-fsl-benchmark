...
def main(args):
    ...
    print(f"Dataset: {args.target_dataset}")
    print("-----------------------------------")
    prompt_csv_path = f"{args.prompt_dir}/{args.target_dataset}.csv"
    prompt_df = pd.read_csv(prompt_csv_path)
    # iterate through the rows of the csv
    for index, row in prompt_df.iterrows():
        prompts = row.to_dict()
        print(prompts)
        args.prompt_index = index
...
def ft_and_test(args, write_csvs=True):
    ...
    experiment_name = f"{args.backbone.replace('/', '__')}/he{args.prompt_index}"

    csv_logger = CSVLogger(save_dir, name=experiment_name, flush_logs_every_n_steps=1)

    # prompts_in_use = {
    #     k: args.prompts[k]
    #     for k in metatest_dataset.task_source.task_description.classes.values()
    # }
    prompts_in_use = args.prompts
    print("Prompts in use:", prompts_in_use)

    is_multilabel = (
        metatest_dataset.task_source.task_description.task_target
    ...
    model = CLIPClassifier(
        ...
        class_prompts=list(prompts_in_use.values()),
        nothing_prompt_is_last=is_multilabel,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prompt_dir", type=str, default="data/class_prompts")
